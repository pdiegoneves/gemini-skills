...
A skill for AI coding agents that helps create comprehensive Technical Design Documents (TDDOCs) following industry standards.
...
- **User's language**: Automatically generates TDDOC in Portuguese, English, or Spanish based on your request

## How to Use

### Basic Usage

Simply ask the AI agent to create a TDDOC:

**English:**

```
Create a TDDOC for Stripe payment integration
```
...
```
Create a TDDOC for integrating Stripe payments into our subscription system
```
...
5. Agent generates comprehensive TDDOC with all required sections

**Result:** A complete TDDOC with:
...
3. Agent generates streamlined TDDOC with essential sections only

**Result:** A focused TDDOC with:
...
```
Crie um TDDOC para migração do banco de dados PostgreSQL para MongoDB
```

**What Happens:**

1. Agent detects Portuguese language → generates TDDOC in Portuguese
...
**Result:** A TDDOC in Portuguese with:
...
```
Create a TDDOC for Stripe
```

Try:

```
Create a TDDOC for integrating Stripe payments. We need to support subscriptions,
...
```
Create a TDDOC for user authentication. In scope: email/password, JWT tokens,
...
```
Create a TDDOC for the database migration project. This is a large project
(expected 2 months).
```
...
```
Create a TDDOC for Stripe integration. We need PCI DSS compliance, webhook
...
- **English**: "Create a TDDOC for..."
- **Portuguese**: "Crie um TDDOC para..."
- **Spanish**: "Crea un TDDOC para..."
...
| ---------- | ----------------------------------------- |
| English    | "Create a TDDOC for Stripe integration"     |
| Portuguese | "Crie um TDDOC para integração com Stripe"  |
| Spanish    | "Crea un TDDOC para integración con Stripe" |
...
After generating a TDDOC, the agent will offer to publish it to Confluence:
...
The TDDOC includes a metadata section for Epic/Ticket links. You can manually add these or ask the agent to help create Jira tickets.
...
The TDDOC documents **decisions and contracts**, not code:
...
The agent will **insist** on completing mandatory sections before finalizing the TDDOC. You can't skip:
...
**This is normal!** The skill is designed to gather complete information. Answer the questions to get a comprehensive TDDOC.
...
### The TDDOC is Missing Something
...
## Next Steps After Creating a TDDOC

1. **Review**: Check all sections are complete
2. **Share**: Get feedback from team members
3. **Approve**: Get sign-off from stakeholders
4. **Implement**: Use the TDDOC as a guide during development
5. **Update**: Keep the TDDOC updated as the project evolves
